...
async function askInstallPermissions(app, action = 'install') {
  ...
  if (app?.slug) {
    const catalog = findFreshCatalogItem(app.slug)
...
function findFreshCatalogItem(slug) {
  return appStore.catalogs.draft.items.find((item) => item.slug === slug)
    ?? appStore.catalogs.store.items.find((item) => item.slug === slug)
    ?? appStore.findCatalogItem(slug)
}
...
function syncPublisherVersionBadgesFromCatalog() {
  for (const userApp of shell.state.userApps) {
    ...
    const catalog = findFreshCatalogItem(userApp.slug)
    if (!catalog) continue
